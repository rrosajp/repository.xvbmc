...
def resolve_video_urls_for_playing(server, url, video_password="", muestra_dialogo=False):
    ...
    # Averigua la URL del vídeo
        ...
        if server_parameters:
            ...
            if muestra_dialogo:
                progreso = platformtools.dialog_progress("dss", "Conectando con %s" % server_parameters["name"])
...
def get_server_setting(name, server, default=None):
    """
        ...
        Busca en la ruta \addon_data\plugin.video.dss\settings_servers el archivo server_data.json y lee 
