...
def save_library_movie(item):
    ...
    # progress dialog
    p_dialog = platformtools.dialog_progress('dss', 'Añadiendo película...')
...
def save_library_episodes(path, episodelist, serie, silent=False, overwrite=True):
    ...
    if not silent:
        # progress dialog
        p_dialog = platformtools.dialog_progress('dss', 'Añadiendo episodios...')
    ...
    if not insertados and not sobreescritos and not fallidos:
        ...
        if config.is_xbmc():
            if config.get_setting("sync_trakt_new_tvshow", "biblioteca"):
                ...
                # Se lanza la sincronización para la biblioteca de dss
                xbmc_library.sync_trakt_dss(path)
