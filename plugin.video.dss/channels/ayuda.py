...
# ----------------------------------------------------------------------
# dss - XBMC Plugin
# ayuda - Videos de ayuda y tutoriales para dss
...
def mainlist(item):
    ...
    if not config.is_xbmc():
        ...
        if not config.get_setting("premium", server="realdebrid"):
            title = "Activar cuenta real-debrid (Marca la casilla en la ventana de configuración de dss para continuar)"
            action = ""
        elif token_auth:
...
def faq(item):
    ...
    if item.extra == "filtrar_enlaces":
        respuesta = platformtools.dialog_yesno("dss",
        ...
        text = ("Puede que no hayas escrito la ruta de la librería correctamente en "
                ...
                "ya que es un problema ajeno a dss que existe desde hace mucho.\n"
        ...
        text = ("Puede ser que la página web del canal no funcione. "
                ...
                "repositorio de GitHub (github.com/dss-ce/dss-ce). "
...
def get_thumbnail_path(thumb_name):
    import urlparse
    web_path = "https://raw.githubusercontent.com/dss-ce/media/master/dss/squares/"
...
def tutoriales(item):
    ...
    for playlist in playlists:
        if playlist.title == "Tutoriales de dss":
...
def force_creation_advancedsettings(item):
    ...
    advancedsettings_dss = os.path.join(config.get_runtime_path(), "resources",
    ...
    if platformtools.dialog_yesno("AVISO",
        ...
        if os.path.exists(advancedsettings_kodi) and orig_size != 0:
            ...
            if platformtools.dialog_yesno("dss",
                ...
                if not os.path.exists(fichero_backup):
                    ...
                    for line in f_origen:
                        f_backup.write(line)
                    f_backup.close()
                    platformtools.dialog_notification("dss",
                                                      "Backup creado")
                ...
                f_mod = open(os.path.join(advancedsettings_dss))
                ...
                import filecmp
                if filecmp.cmp(advancedsettings_dss, advancedsettings_same):
                    platformtools.dialog_ok("dss",
            ...
            f_optimo = open(advancedsettings_dss)
    ...
    if platformtools.dialog_yesno("dss",
        ...
        if os.path.exists(fichero_backup):
            ...
            if platformtools.dialog_yesno("dss",
                ...
                platformtools.dialog_notification("dss", "Backup completado")
            else:
                platformtools.dialog_notification("dss", "Backup no creado")
