...
def sync_trakt_dss(path_folder):
    """
    ...
    if xbmc.getCondVisibility('System.HasAddon("script.trakt")'):
        ...
        # obtenemos los valores de la serie de dss
...
def search_library_path():
    sql = 'SELECT strPath FROM path WHERE strPath LIKE "special://%/plugin.video.dss/library/" AND idParentPath ISNULL'
...
def ask_set_content():
    ...
    if config.get_setting("library_ask_set_content") == 1 and config.get_setting("library_set_content") == False:
        heading = "dss Auto-configuración"
        linea1 = "¿Desea que dss auto-configure la biblioteca de Kodi?"
