...
def dialog_yesno(heading, line1, line2="", line3="", nolabel="No", yeslabel="Yes", autoclose=""):
    dialog = xbmcgui.Dialog()
    if autoclose:
...
def set_context_commands(item, parent_item):
    ...
    for command in context:
        ...
        if type(command) == dict:
            # The parameters of the dict, are overwritten to the new context_item in case of overwriting "action" and
    ...
    # Options according to criteria, only if the item is not a tag (label), nor is it "Add to the library", etc ...
    if item.action and item.action not in ["add_movie_to_library", "add_serie_to_library", "search trailer"]:
        ...
        if item.infoLabels['plot'] and (num_version_xbmc < 17.0 or item.contentType == 'season'):
            context_commands.append(("information", "XBMC.Action(Info)"))
        ...
        if parent_item.channel not in ["novedades", "channelselector"] and item.action != "mainlist" \
            ...
            context_commands.append(("Go to the Main Menu", "XBMC.Container.Refresh (%s?%s)" %
        ...
        if item.channel != "biblioteca":
            ...
            if item.action in ["episodios", "get_episodios"] and item.contentSerieName:
                context_commands.append(("Add Series to Library", "XBMC.RunPlugin(%s?%s)" %
            ...
            # Añadir Pelicula a Biblioteca
                ...
                context_commands.append(("Add Film to Library", "XBMC.RunPlugin(%s?%s)" %
        ...
        if item.channel != "descargas":
            ...
            if item.contentType == "movie" and item.contentTitle:
                context_commands.append(("Download Movie", "XBMC.RunPlugin(%s?%s)" %
                ...
                if item.contentType == "tvshow":
                    context_commands.append(("Download Series", "XBMC.RunPlugin(%s?%s)" %
                                             (sys.argv[0], item.clone(channel="descargas", action="save_download",
                                                                      from_channel=item.channel,
                                                                      from_action=item.action).tourl())))
                ...
                if item.contentType == "episode":
                    context_commands.append(("Download Episode", "XBMC.RunPlugin(%s?%s)" %
                                             (sys.argv[0], item.clone(channel="descargas", action="save_download",
                                                                      from_channel=item.channel,
                                                                      from_action=item.action).tourl())))
                ...
                if item.contentType == "season":
                    context_commands.append(("Download Season", "XBMC.RunPlugin(%s?%s)" %
                                             (sys.argv[0], item.clone(channel="descargas", action="save_download",
                                                                      from_channel=item.channel,
                                                                      from_action=item.action).tourl())))
        ...
        if parent_item.channel not in ["configuracion", "novedades", "buscador"]:
            context_commands.append(("Open Configuration", "XBMC.Container.Update(%s?%s)" %
...
def handle_wait(time_to_wait, title, text):
    ...
    while secs < time_to_wait:
        ...
        remaining_display = ' Wait ' + secs_left + ' Wait seconds before the video starts...'
...
def play_torrent(item, xlistitem, mediaurl):
    ...
    if len(torrent_options) > 1:
        seleccion = dialog_select("Open torrent with...", [opcion[0] for opcion in torrent_options])
    else:
        seleccion = 0
    ...
    if seleccion == 0:
        ...
        c = Client(url=mediaurl, is_playing_fnc=xbmc.Player().isPlaying, wait_time=None, timeout=10,
                   temp_path=os.path.join(client_tmp_path, "dss-torrent"), print_status=debug)

        # Mostramos el progreso
        progreso = dialog_progress("dss - Torrent", "Starting...")
        ...
        while not c.closed:
            try:
                ...
                if debug:
                    ...
                    try:
                        txt3 = 'Stopping automatically in: %ss' % (int(s.timeout))
                    except:
                        txt3 = ''
                ...
                if progreso.iscanceled():
                    progreso.close()
                    if s.buffer == 100:
                        if dialog_yesno("dss - Torrent", "¿Deseas iniciar la reproduccion?"):
                            played = False
                            progreso = dialog_progress("dss - Torrent", "")
                            progreso.update(s.buffer, txt, txt2, txt3)
                        else:
                            progreso = dialog_progress("dss - Torrent", "")
                            break
                        ...
                        if dialog_yesno("dss - Torrent", "¿Deseas cancelar el proceso?"):
                            progreso = dialog_progress("dss - Torrent", "")
                            break
