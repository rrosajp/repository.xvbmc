        ...
        if self.items:
            ...
            if not self.standalone:
                ...
                if self.livescoresdata[-1]["prev"]:
                    self.getControl(33501).setLabel("<< Yesterday")
                    self.prev = self.livescoresdata[-1]["prev"]
                if self.livescoresdata[-1]["next"]:
                    self.getControl(33502).setLabel("Next Day >>")
                    self.next = self.livescoresdata[-1]["next"]

                self.getControl(33503).setLabel("ALL (%s)" % str(fin+juego+por_empezar))
                self.getControl(33504).setLabel("In Game (%s)" % str(juego))
                self.getControl(33505).setLabel("Not Starting (%s)" % str(por_empezar))
                self.getControl(33506).setLabel("Finished (%s)" % str(fin))
