...
def get_news(url=""):
    idiomas = [["nl", "noticias"], ["www", "news"], ["fr", "infos"], ["pt", "noticias"]]
...
def get_agenda(url="deporte", data_canales=""):
    data = httptools.downloadpage("http://www.ukfootballontv.co.uk//%s" % url, cookies=False).data
    data = re.sub(r"\n|\r|\t", '', data)

    if not data_canales:
        data_canales = httptools.downloadpage("http://www.ukfootballontv.co.uk/channel/", cookies=False).data
