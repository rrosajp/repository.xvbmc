...
class Untangle:
    ...
    def __init__(self, value='', file_name='', type_video="MOVIE", use_url=True, use_meta_info=True, imdb_id='',
                 icon='', fanart='', tmdb_api='', storage_path=''):
        ...
        # getting IMDB code
        if self.imdb_id is '':
            self.id = find_imdb(self.proper_title, self.storage_path)
            self.imdb_id = self.id

        # Reading infoLabels
        self.info_labels = self.get_info_labels(storage_path=storage_path)
        ...
        if self.info_title["type"] == 'SHOW':  # difference with show and anime
            self.info = self.get_info_episode(self.info_labels, storage_path=storage_path)
    ...
    def get_info_episode(self, info_labels_init, storage_path=""):
