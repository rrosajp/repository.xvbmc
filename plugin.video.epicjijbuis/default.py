...
epicaddBas='aHR0cDovL2JpdC5seS8='
epicaddDir=base64.b64decode('RVBpQy1KaWpCdWlz')
...
icons=re.compile('.+?picthumb="(.+?)"').findall(word)
epicarts=re.compile('.+?picart="(.+?)"').findall(word)
nfo=re.compile('.+?picinfo="(.+?)"').findall(word)
def addonIcon():
...
def addDir(name,url,mode,iconimage,epicart,description):
 u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)+"&iconimage="+urllib.quote_plus(iconimage)+"&epicart="+urllib.quote_plus(epicart)+"&description="+urllib.quote_plus(description)
 ok=True
 liz=xbmcgui.ListItem(name,iconImage="DefaultFolder.png",thumbnailImage=iconimage)
 liz.setInfo(type="Video",infoLabels={"Title":name,"Plot":description})
 liz.setProperty("Fanart_Image",epicart)
 if mode==1:
...
def main_list(params):
 setView('movies','EPiC')
 plugintools.log("EPiC-XvBMC.main_list "+repr(params))
 ...
 addDir('',BASEURL,2,icondir,fanartdir,'')
 for name,url,mode,iconimage,epicart,description in zip(names,urls,modus,icons,epicarts,nfo):
  plugintools.log("EPiC_mode "+str(mode))
  addDir(name=name,url=url,mode=mode,iconimage=iconimage,epicart=epicart,description=description)
 addDir('',BASEURL,2,icondir,fanartdir,'')
...
def NoxMusic():
 setView('movies','EPiC')
 plugintools.log("EPiC-XvBMC.Nox_list")
 addDir(whoami,BASEURL,2,icondir,fanartdir,'')
 addDir('',BASEURL,2,icondir,fanartdir,'')
...
def randomizer(params):
 setView('movies','EPiC')
 plugintools.log("EPiC-XvBMC.Random_list ")
 addDir(whoami,BASEURL,2,icondir,fanartdir,'')
 addDir('',BASEURL,2,icondir,fanartdir,'')
...
def xvbmcnl(params):
 setView('movies','EPiC')
 plugintools.log("EPiC-XvBMC-NL_list ")
 addDir(whoami,BASEURL,2,icondir,fanartdir,'')
 addDir('',BASEURL,2,icondir,fanartdir,'')
...
def joetjoeb(params):
 setView('movies','EPiC')
 plugintools.log("EPiC-XvBMC.JoeTjoeb_list ")
 addDir(whoami,BASEURL,2,icondir,fanartdir,'')
 addDir('',BASEURL,2,icondir,fanartdir,'')
